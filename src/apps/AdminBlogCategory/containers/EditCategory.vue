...
<template>
  ...
  >
    <Form
      ...
      :label-width="120"
    >
      <FormItem :label="$t('L00172')" prop="name">
        <Input v-model="modalForm.name" :placeholder="$t('L00173')" style="width: 200px;"></Input>
      </FormItem>
      <FormItem :label="$t('L00174')" prop="alias">
        <Input v-model="modalForm.alias" :placeholder="$t('L00175')" style="width: 200px;"></Input>
      </FormItem>
      <FormItem :label="$t('L00176')" prop="description">
        <Input v-model="modalForm.description" :placeholder="$t('L00177')" type="textarea" :rows="4" style="width: 300px"></Input>
      </FormItem>
...
<script>
  export default {
    ...
    methods: {
      ...
      doSave: async function () {
        ...
        if (_t.currentAction === 'add') {
          actionPath = 'Apps/AdminBlogCategory/add'
        } else if (_t.currentAction === 'edit') {
          actionPath = 'Apps/AdminBlogCategory/edit'
        }
        if (!actionPath) {
          return
        }
    ...
    created: async function () {
      let _t = this
      // 监听
      _t.$X.utils.bus.$on('Apps/AdminBlogCategory/edit', async function (data) {
        // 显示弹窗
        _t.isShowModal = true
        // 备份数据
    ...
    beforeDestroy: function () {
      let _t = this
      // 销毁监听
      _t.$X.utils.bus.$off('Apps/AdminBlogCategory/edit')
    }
  }
</script>
